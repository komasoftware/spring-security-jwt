...
public class GoogleAuthorizationResponseServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		tokenAuthenticationService = applicationContext
	...
	protected void doGet(HttpServletRequest request,
		...
		String expectedToken = (String) request.getSession().getAttribute(
				XsrfUtils.XSRF_KEY);
		...
		GoogleTokenResponse tokenResponse = exchangeCodeForAccessAndRefreshTokens(
				code[0], requestUrl);
	...
	private GoogleTokenResponse exchangeCodeForAccessAndRefreshTokens(
			String code, String currentUrl) throws IOException {
		return new GoogleAuthorizationCodeTokenRequest(HTTP_TRANSPORT,
				JSON_FACTORY, appConfig.getGoogleClientId(),
				appConfig.getGoogleClientSecret(), code, currentUrl).execute();
	}

	private static String getOAuthCodeCallbackHandlerUrl(
	...
	private String establishUserAndLogin(HttpServletResponse response,
		...
		try {
			...
			user = new User(email, UUID.randomUUID().toString(),
		...
		return tokenAuthenticationService.addAuthentication(response,
				authentication);
	}
}
